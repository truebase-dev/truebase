...
function checkWashSale(exitDate: string, exitPrice: number) {
  ...
  // Flags true if a purchase exists within 30 days of the exit date at a higher price (simplified)
...
function recalculateLiveMetrics() {
  ...
  return { currentFeeRate, dynamicDCA: totalRemainingTokens > 0 ? (totalSpentOnRemaining / totalRemainingTokens) : 0.51, totalRemainingTokens };
}
...
app.get('/api/analytics', (c) => {
  ...
  return c.json({
    success: true,
    metrics: { thirtyDayVolume: globalVolume, currentFee: metrics.currentFeeRate, dynamicDCA: metrics.dynamicDCA, totalTokensAccumulated: metrics.totalRemainingTokens },
...
app.post('/api/transactions', async (c) => {
  ...
  if (type === 'Profit-Taking Exit') {
    washFlag = checkWashSale(date, parsedPrice);
    // ... (Lot exhaustion logic remains same) ...
  }
  
  // Logic to process transaction...
  return c.json({ success: true, washSaleDetected: washFlag });
});
